...
def build_semantic_calculation_prompt(
    ...
    return (
        ...
        "An evidence item with kind hold_precondition_resolution and "
        "resolution_mode ALLOW_CONDITIONAL_PROCEEDING resolves only the "
        "operator's scope decision. It establishes no fact, authority, or "
        "missing evidence; preserve every supplied unknown. Reclassify an "
        "unknown as UNRESOLVABLE_LIMITATION only when the signed decision makes "
        "it no longer operator-resolvable in this cycle and a bounded semantic "
        "candidate is safe. Do not use that decision to bypass a formal "
        "predicate input, norm-linked or Core-linked evidence requirement, "
        "STOP boundary, authority requirement, or mandatory proof. The decision "
        "never forces PASS; recalculate the same phase gate from the retained "
        "material. "
