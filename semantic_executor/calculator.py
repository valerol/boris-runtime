...
def build_semantic_calculation_prompt(
    ...
    return (
        ...
        "phase, norm_results, unknowns, uncertainties, conflicts, alternatives, "
        "suggested_gate, candidate_result. Copy core_ref and phase exactly. "
        "Return exactly one "
        ...
        "Every top-level or per-norm unknown must have exactly one matching "
        "typed item in uncertainties. Each uncertainty item must contain exactly "
        "uncertainty_id, description, resolution_class, target_path, norm_refs, "
        "core_refs, operator_question. description must copy the unknown text "
        "exactly. resolution_class must be exactly one of OPERATOR_INPUT, "
        "RUNTIME_DERIVABLE, FUTURE_CONTINGENT, MODEL_UNCERTAINTY, "
        "DOWNSTREAM_PRECONDITION, UNRESOLVABLE_LIMITATION. Use OPERATOR_INPUT "
        "only for a missing input that the operator can actually know and "
        "provide now; set a direct operator_question. Never use it for an "
        "internal cycle object, epistemic assessment, provenance or inference "
        "trace produced by Runtime, a future event, a behavioral/model "
        "uncertainty, or a later gate-admission requirement. For "
        "RUNTIME_DERIVABLE and DOWNSTREAM_PRECONDITION, cite one or more exact "
        "resolution_ref values from semantic_view."
        "uncertainty_resolution_catalog in core_refs. Copy the catalog's "
        "resolution_class; do not reinterpret its ownership. Use null "
        "operator_question for every non-operator class. target_path may be null "
        "when the uncertainty is not one operator-supplied value. Current-phase "
        "output objects and their schema fields are calculator work: form their "
        "semantic content in candidate_result when the supplied material permits "
        "it, rather than requesting those objects from the operator. GateEvidence, "
        "runtime registries, independent review and admission artifacts are "
        "downstream preconditions and do not authorize execution. Future and "
        "model uncertainty must become explicit conditions, scenarios, or "
        "confidence limits in candidate_result rather than fabricated facts. "
        "semantic_view.operator_decision, when present, is a Runtime-verified "
        "current-cycle operator decision. It is not a persisted fact, evidence "
        "item, authority grant, or memory record. Use its values only for the "
        "signed target paths and preserve that distinction in candidate_result. "
        "PROVIDE_INFORMATION resolves only its declared unknown IDs. "
        "CONFIRM_ASSUMPTION preserves unknowns and makes its values explicit "
        "assumptions. ALLOW_CONDITIONAL_PROCEEDING preserves every unknown and "
        "removes only the signed conditionally_non_blocking targets from the "
        "current gate's blocking set. CHANGE_SCOPE changes only the supplied "
        "semantic selectors. No OperatorDecision forces PASS, bypasses STOP, "
        "creates authority, or supplies mandatory proof; recalculate the same "
        "phase gate from the retained material. "
        "Do not suggest HOLD solely because a FUTURE_CONTINGENT, "
        "MODEL_UNCERTAINTY, DOWNSTREAM_PRECONDITION, or "
        "UNRESOLVABLE_LIMITATION exists when a safe conditional semantic "
        "candidate can be formed. Preserve any independently required STOP or "
        "REPAIR. "
